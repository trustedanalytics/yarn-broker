...
package org.trustedanalytics.servicebroker.yarn.config.catalog;
  ...
  public Catalog catalog() {
    return new Catalog(Arrays.asList(
        new ServiceDefinition(configuration.getCfServiceId(), configuration.getCfServiceName(),
            "YARN service allow to access resources on hadoop cluster.", true, true, getSharedPlans(), null, getServiceDefinitionMetadata(),
  ...
  private List<Plan> getSharedPlans() {
    return Lists.newArrayList(
        new Plan(configuration.getCfBaseId() + "-bare-plan", "bare",
            "This Plan provides YARN configuration.", getPlanMetadata(false), true));
  }
